...
if (isset($_POST['punch_in'])) {
    // Record punch in
    $insertQuery = "INSERT INTO attendance (employee_id, date, punch_in) VALUES (?, ?, NOW())";
...
if (isset($_POST['punch_out'])) {
    // Record punch out
     // Record punch in
     $insertQuery = "INSERT INTO attendance (employee_id, date, punch_out) VALUES (?, ?, NOW())";
     $stmt = $conn->prepare($insertQuery);
     $stmt->bind_param("is", $employee_id, $today);
 
    // Your punch out logic here
