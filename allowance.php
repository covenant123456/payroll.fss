...
$sql_allowances = "SELECT type, amount FROM payable";
$result_allowances = $conn->query($sql_allowances);

// Check if there are allowances to calculate
if ($result_allowances->num_rows > 0) {
    // Loop through each allowance
    while ($row = $result_allowances->fetch_assoc()) {
        // Retrieve the allowance type and percentage
        $allowance_type = $row['type'];
        $allowance_percentage = $row['amount'];

        // Check if the allowance already exists for the employee
        $sql_check = "SELECT * FROM allowance WHERE employee_id = $employee_id AND type = '$allowance_type'";
        $result_check = $conn->query($sql_check);

        // If allowance doesn't exist, calculate and insert it
        if ($result_check->num_rows == 0) {
            // Calculate the amount for the current allowance based on the percentage and employee salary
            $calculated_amount = $salary * ($allowance_percentage / 100);

            // Insert the calculated allowance into the allowances table
            $sql_insert_allowance = "INSERT INTO allowance (employee_id, type, amount) VALUES ($employee_id, '$allowance_type', $calculated_amount)";
            if ($conn->query($sql_insert_allowance) === TRUE) {
                echo "Allowance inserted successfully for type: $allowance_type<br>";
            } else {
                echo "Error inserting allowance: " . $conn->error . "<br>";
            }
        }
    }

    // Reset the internal pointer of the result set to the beginning
    $result_allowances->data_seek(0);
} else {
                ...
                <div class="container-3">
                    ...
                    <div class="paid-info">
                        <div class="container-5">
                            ...
                            if ($result_allowances->num_rows > 0) {
                                // Loop through each allowance
                                while ($row_allowance = $result_allowances->fetch_assoc()) {
                                    // Retrieve the allowance details
                                    $allowance_name = $row_allowance["type"];
                                    $allowance_percentage = $row_allowance["amount"];
                                    $amount = ($salary * $allowance_percentage) / 100;
                                    // Display the details of each allowance
                                    echo '<div class="div-7">';
                                    echo '<div class="text-wrapper-14">' . $allowance_name . '</div>';
                                    echo '<div class="information">';
                                    echo '<div class="text-wrapper-15"></div>';
                                    echo '<div class="text-wrapper-7"></div>';
                                    echo '</div>';
                                    echo '<div class="information">';
                                    echo '<div class="text-wrapper-16">' . $allowance_percentage . '%</div>';
